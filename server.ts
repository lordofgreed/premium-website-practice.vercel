...
async function startServer() {
  ...
  // Fallback JS chat logic if Python is unavailable
  function getFallbackJsReply(message: string): string {
    const lowerMsg = message.toLowerCase();
    if (lowerMsg.includes("shipping") || lowerMsg.includes("delivery")) {
      return "We offer free standard global shipping on all premium vault items! Delivery usually takes 3-5 business days depending on your region.";
      ...
      return "All OGhaitong gear comes with a 1-year limited hardware warranty covering manufacturing defects.";
    } else if (lowerMsg.includes("contact") || lowerMsg.includes("human")) {
      return "You can reach our human support squad at support@oghaitong.com or open a priority ticket in your account console.";
    } else if (lowerMsg.includes("esports") || lowerMsg.includes("gaming") || lowerMsg.includes("mobile legends")) {
      return "Looking to gear up for the FAI8 Pro League or AuraHILLS community scrims? Check out our Gaming section for tournament-grade tactical mobile triggers and active cryo-cooling accessories perfect for Mobile Legends: Bang Bang!";
    } else if (lowerMsg.includes("audio") || lowerMsg.includes("headphone") || lowerMsg.includes("latency")) {
      return "Our Aura Unleashed Pro series headphones feature dynamic hybrid ANC, 40-hour battery life, and Bluetooth 5.4 zero-latency connection.";
    } else if (lowerMsg.includes("hello") || lowerMsg.includes("hi ") || lowerMsg.includes("hey")) {
      return "Hello there! I'm your digital support assistant. Feel free to ask me about shipping, returns, warranty, or gear specifications!";
    }
    return "Thanks for reaching out! I couldn't find an exact match for that question. For orders, specific shipping tracking, or custom inquiries, please contact our human team directly at support@oghaitong.com.";
  }

  // Python-backed chatbot API endpoint
  app.post('/api/chat', (req, res) => {
    ...
    const pyProcess = spawn(pythonCmd, [path.join(process.cwd(), 'chatbot.py'), message]);
    ...
    pyProcess.on('close', (code) => {
      if (code !== 0) {
        console.warn(`Python chatbot.py exited with code ${code}. Stderr: ${stderrData}. Using fallback JS logic.`);
        const reply = getFallbackJsReply(message);
        return res.json({ reply, source: 'javascript_fallback' });
      }

      try {
        const result = JSON.parse(stdoutData.trim());
        return res.json({ reply: result.reply, source: 'python_chatbot' });
      } catch (err) {
        console.error('Failed to parse Python chatbot output:', stdoutData);
        const reply = getFallbackJsReply(message);
        return res.json({ reply, source: 'javascript_fallback' });
      }
    });
    ...
    pyProcess.on('error', (err) => {
      console.warn('Could not spawn Python process. Using fallback JS logic.', err.message);
      const reply = getFallbackJsReply(message);
      return res.json({ reply, source: 'javascript_fallback' });
    });
  });
