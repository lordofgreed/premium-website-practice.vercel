...
export default function StoreFront({
  ...
  const [chatMessages, setChatMessages] = useState([
    { role: 'bot', text: '👋 **Hello there, champion!**\n\nI\'m your official **OGhaitong Support AI**. Type **help** to see all available options, or ask me anything about shipping, gaming gear, warranty, and headphones!' }
  ]);
  ...
  // =======================================================================
  // ONLINE PYTHON-BACKED CUSTOMER SUPPORT CHATBOT LOGIC
  // =======================================================================
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Send message to our Express API proxying the Python if-else chatbot
      ...
      setChatMessages(prev => [
        ...prev, 
        { role: 'bot', text: 'Our customer support systems are currently undergoing brief maintenance. Please try again or reach out to support@oghaitong.com.' }
      ]);
    } finally {
          ...
          <div className="mb-4 w-80 sm:w-96 bg-slate-950 border border-white/10 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col animate-slide-up origin-bottom-right">
            ...
            <div className="h-80 p-4 overflow-y-auto bg-slate-900/50 flex flex-col space-y-3 scrollbar-thin scrollbar-thumb-slate-700">
              {chatMessages.map((msg, idx) => {
                ...
                // Helper to render simple formatted paragraphs and bullets
                const renderFormattedText = (text: string) => {
                  ...
                  return lines.map((line, i) => {
                    ...
                    const parsedLine = parts.map((part, index) => {
                      if (index % 2 === 1) {
                        return <strong key={index} className="font-extrabold text-cyan-300">{part}</strong>;
                      }
                      return part;
                    });
                    ...
                    if (isBullet) {
                      return (
                        <div key={i} className="flex items-start gap-1.5 my-1 ml-1 text-slate-300">
                          <span className="text-cyan-400 shrink-0 mt-1 text-xs">•</span>
                          <span className="text-sm leading-relaxed">{parsedLine}</span>
                        </div>
                      );
                    }
                    
                    return (
                      <p key={i} className="leading-relaxed min-h-[0.5rem]">
                        {parsedLine}
                      </p>
                    );
                  });
                ...
                return (
                  <div 
                    ...
                    className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm ${
                      isUser 
                        ? 'bg-cyan-500 text-slate-950 self-end rounded-tr-sm font-semibold' 
                        : 'bg-slate-800 text-slate-200 self-start border border-slate-700 rounded-tl-sm'
                    }`}
                  >
                    {isUser ? (
